      ...
      >
        {slides.map((slide, index) => (
          <SwiperSlide key={index}>
            <img
              className={cn(
                `aspect-square w-full rounded-lg object-fill ${slideClassName}`,
              )}
              src={slide}
            />
          </SwiperSlide>
        ...
        >
          <IcCircleArrowLeft className='text-[#0360AE]'/>
        </div>
        ...
        >
          <IcCircleArrowLeft className='rotate-180 text-[#0360AE]'/>
        </div>
      </Swiper>

      <div
        className={cn(
          `custom-pagination-${id} mt-4 flex items-center justify-center [&>.swiper-pagination-bullet]:h-3 [&>.swiper-pagination-bullet]:w-3 [&>.swiper-pagination-bullet]:bg-[#0360AE]`,
        )}
      />
    </>
  );
